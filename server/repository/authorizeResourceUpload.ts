...
function validateFilename(filename: string) {
  const extension = filename.split(".").pop()?.toLowerCase() as
    ResourceUploadFileExtension | undefined;
  const definition = extension
    ? resourceUploadFileDefinitions[extension]
    : undefined;
  if (!definition) {
...
export async function authorizeResourceUpload(
  input: ResourceUploadRequest,
  ...
  if (
    input.categoryId &&
    !section.categories.some((category) => category.id === input.categoryId)
  ) {
    throw new InvalidResourceUploadError(
  ...
  if (await objectExists(config.bucketName, key))
    throw new DuplicateResourceError(
      "A resource with this repository key already exists.",
    );

  const createUploadUrl =
    dependencies.createUploadUrl ??
    (async (_config, bucket, objectKey, mimeType, fileSize) =>
      getSignedUrl(
        client,
